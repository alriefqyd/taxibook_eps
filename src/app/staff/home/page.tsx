...
function staffBuildWaMessage(b: any): string {
  ...
  return [
    ...
    `━━━━━━━━━━━━━`,
    `🔖 *${b.booking_code}*`,
    `👤 Passenger: *${b.passenger_name || 'You'}*`,
    `📍 From: ${b.pickup}`,
    `🏁 To: *${b.destination}*`,
    `🕐 Schedule: ${time}`,
    `🚗 Type: ${type}`,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Action buttons — call, WhatsApp, cancel stacked like a menu */}
        {booking.driver_phone && !showCancel && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: canCancel ? 8 : 0 }}>
            <a
              href={`tel:${booking.driver_phone}`}
              style={{ width: '100%', padding: '12px', background: '#EFF6FF', color: '#0369A1', border: '1px solid #BAE6FD', borderRadius: 16, fontSize: '13px', fontWeight: 700, cursor: 'pointer', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxSizing: 'border-box' }}
            >
              ...
              Call {booking.driver_name || 'driver'}
            </a>
            <a
              href={`https://wa.me/${staffToWaNumber(booking.driver_phone)}?text=${encodeURIComponent(staffBuildWaMessage(booking))}`}
              target="_blank" rel="noopener noreferrer"
              style={{ width: '100%', padding: '12px', background: '#25D366', color: '#ffffff', border: 'none', borderRadius: 16, fontSize: '13px', fontWeight: 700, cursor: 'pointer', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxSizing: 'border-box' }}
            >
              ...
              WhatsApp {booking.driver_name || 'driver'}
            </a>
          </div>
        )}
